...
export default function BottomNav() {
  ...
  return (
    <nav className="sm:hidden fixed bottom-0 inset-x-0 bg-card border-t border-border flex justify-around pt-2.5 pb-[calc(0.625rem+env(safe-area-inset-bottom))] z-10">
      <Link
        to="/feed"
        className={`flex flex-col items-center gap-0.5 text-[9px] font-medium ${navCls('/feed')}`}
        aria-label="Feed"
      >
      ...
      <Link
        to="/search"
        className={`flex flex-col items-center gap-0.5 text-[9px] font-medium ${navCls('/search')}`}
        aria-label="Search"
      >
      ...
      <Link
        to="/notifications"
        className={`relative flex flex-col items-center gap-0.5 text-[9px] font-medium ${navCls('/notifications')}`}
        aria-label="Alerts"
      >
        <span className="relative">
          ...
          <span
            ...
            className={`absolute -top-0.5 -right-1.5 min-w-[14px] h-[14px] rounded-full bg-danger text-white text-[8px] font-bold flex items-center justify-center px-0.5 leading-none transition-opacity ${unreadCount > 0 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
          >
      ...
      <Link
        to="/profile"
        className={`flex flex-col items-center gap-0.5 text-[9px] font-medium ${navCls('/profile')}`}
        aria-label="Profile"
      >
